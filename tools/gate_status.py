...
def main() -> None:
    ...
    lines.append(
        f"Generated by `tools/gate_status.py` from the evidence files, "
        f"{stamp}. A gate with no record reads NOT YET RUN rather than being "
        f"omitted."
    )
    lines.append("")
    ...
    if results["G9"] is None or results["G10"] is None:
        ...
        lines.append(
            f"{' and '.join(pending)} {verb} not yet run, and listed rather than "
            "omitted."
        )
        lines.append("")
    ...
    if START in text and END in text:
        ...
        raise SystemExit(
            f"{START} / {END} markers not found in README.md; refusing to write a "
            "gate table into an unknown position."
        )
