...
**[docs/TESTING.md](docs/TESTING.md)** walks a reviewer through the whole loop:
three minutes read-only with no wallet, then halting a target yourself and
trying to make the system lie, then checking the result without trusting this
interface at all.

Four sandbox targets are deployed, registered and already exploited — each one's
claimable total exceeds what was ever deposited, and each still reads `RUNNING`
because nobody has proved it yet. Halt is one-way, so each can be halted exactly
once.

Funding takes one command; Studio Next lets anyone do it:

```bash
curl -s -X POST https://studio-next.genlayer.com/api -H 'content-type: application/json' \
  -d '{"jsonrpc":"2.0","method":"sim_fundAccount","params":["YOUR_ADDRESS",100000000000000000000],"id":1}'
```
...
Generated by `tools/gate_status.py` from the evidence files, 2026-09-16T12:08:40Z. A gate with no record reads NOT YET RUN rather than being omitted.
...
- **G3**, the true claim that halted CISTERN: verdict `EXPLOIT`, target `HALTED` — [`0x3c3dc4455833ef8f…`](https://explorer-studio-dev.genlayer.com/tx/0x3c3dc4455833ef8f30aeda7a9efecbe4254e1e4a6f7147da0ac07c00f3040e2b)
...
- **G4**, the CISTERN write that reverted after the halt: reverted `STANCH_HALTED`, target `HALTED` — [`0xf139ff9e681d8fc6…`](https://explorer-studio-dev.genlayer.com/tx/0xf139ff9e681d8fc65eb3ed81caf119cb691225e2c36425a9f6f9f007740ea3fa)
